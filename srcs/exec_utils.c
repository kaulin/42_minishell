...
/*   Updated: 2024/09/05 12:24:18 by jajuntti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_file(t_data *data, t_file *cur_file, int flag)
{
	if (is_directory(cur_file->file_str) == 1)
		return (oops(data, 1, cur_file->file_str, "Is a directory"));
	if (flag == 1 && access(cur_file->file_str, F_OK) == -1)
		return (oops(data, 1, cur_file->file_str, "No such file or directory"));
	if (flag == 1 && open(cur_file->file_str, O_RDONLY) == -1)
